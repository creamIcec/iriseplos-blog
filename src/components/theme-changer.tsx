// from https://github.com/actifyjs/actify/blob/main/apps/docs/src/components/ThemeChanger.tsx
// licensed under MIT
...
export const ThemeChanger = ({ className }: ThemeChangerProps) => {
  ...
  const initializeTheme = React.useCallback(() => {
    console.log("ThemeChanger: 开始初始化主题");

    // 1. 先尝试从 localStorage 读取保存的主题设置
    const savedMode = getCurrentMode();
    const savedColor = getCurrentSeedColor();
    const savedTheme = getCurrentThemeString();

    console.log("保存的设置:", { savedMode, savedColor, savedTheme });

    if (savedMode && savedColor && savedTheme) {
      // 如果有保存的设置，使用保存的设置
      console.log("使用保存的主题设置");
      setSelectedColorMode(savedMode);
      setHexColor(savedColor);
      updateHctFromHex(savedColor);

      // 重新应用保存的主题（确保正确加载）
      changeColorAndMode(savedColor, savedMode);
      setTheme(savedMode);
    } else {
      // 2. 如果没有保存的设置，使用默认设置
      console.log("使用默认主题设置");
      const defaultColor = "#ECAA2E";
      const defaultMode: ColorMode = "system";

      setSelectedColorMode(defaultMode);
      setHexColor(defaultColor);
      updateHctFromHex(defaultColor);

      // 应用并保存默认主题
      changeColorAndMode(defaultColor, defaultMode);
      setTheme(defaultMode);
    }

    setIsInitialized(true);
    console.log("ThemeChanger: 主题初始化完成");
  }, [setTheme]);
  ...
  React.useLayoutEffect(() => {
    // 只有在已初始化且用户操作时才更新主题
    if (isInitialized && (hue > 0 || chroma > 0 || tone > 0)) {
      const newHexColor = hexFromHct(hue, chroma, tone);
      setHexColor(newHexColor);
      changeColor(newHexColor);
    }
  }, [hue, chroma, tone, isInitialized]);
  ...
  const content = (
    <div
      className={`flex flex-col w-${
        isWide ? 56 : "full"
      } my-3 mx-4 *:[margin-block-end:16px] ${isWide ? null : "p-4"}`}
      ref={panelRef}
    >
      <section className="flex relative">
        <h2 className="text-2xl tracking-tighter leading-none">主题控制</h2>
      ...
      <section className="px-3 rounded-2xl text-on-surface-variant bg-surface-variant [--md-slider-inactive-track-color: var(--md-sys-color-on-surface-variant)]">
        <div className="[margin-block:24px]">
          <Label className="block leading-tight ms-2.5 me-2.5">色调</Label>
          <div className={isWide ? undefined : "flex flex-col"}>
            <Slider
              labeled
              value={hue}
              maxValue={360}
        ...
        <div className="[margin-block:24px]">
          <Label className="block leading-tight ms-2.5 me-2.5">色度</Label>
          <div className={isWide ? undefined : "flex flex-col"}>
            <Slider
              labeled
              maxValue={150}
        ...
        <div className="[margin-block:24px]">
          <Label className="block leading-tight ms-2.5 me-2.5">色相</Label>
          <div className={isWide ? undefined : "flex flex-col"}>
            <Slider
              labeled
              value={tone}
  ...
  return (
    <div className="relative text-on-surface">
      ...
      >
        <Icon className={`${className}`}>palette</Icon>
      </IconButton>
      {isWide ? (
        ...
        open &&
        open && (
          <div className="transition-all duration-300 ease-out fixed bottom-0 left-0 z-[99] w-[100vw] h-[100vh] bg-black/50">
            <div className="absolute bottom-0 left-0 flex flex-col items-center bg-background rounded-t-2xl w-[100vw] max-h-256">
